...
struct matroid_intersection_weighted_spfa_base{
	...
	Compare cmp;
	matroid_intersection_weighted_spfa_base(F1 check1, F2 insert1, F3 clear1, F4 check2, F5 insert2, F6 clear2, Compare cmp): check1(check1), insert1(insert1), clear1(clear1), check2(check2), insert2(insert2), clear2(clear2), cmp(cmp){ }
	...
	bool augment(const vector<T> &cost){
		...
		for(auto u = 0; u < n; ++ u) if(!used[u]){
			...
			if(check2(u)) adj[u].push_back({n, 0});
		}
		for(auto u = 0; u < n; ++ u) if(used[u]){
			clear1(), clear2();
			for(auto v = 0; v < n; ++ v) if(u != v && used[v]) insert1(v), insert2(v);
			for(auto v = 0; v < n; ++ v) if(!used[v]){
				if(check1(v)) adj[u].push_back({v, cost[v]});
				if(check2(v)) adj[v].push_back({u, -cost[u]});;
			}
		}
		while(beg < (int)q.size()){
			...
			for(auto [v, w]: adj[u]){
				...
				if(cmp(dist_next.first, dist[v].first) || !cmp(dist[v].first, dist_next.first) && dist_next.second < dist[v].second){
					dist[v] = dist_next;
					pv[v] = u;
					if(!in_q[v]){
						q.push_back(v);
...
auto make_matroid_intersection_weighted_spfa(F1 check1, F2 insert1, F3 clear1, F4 check2, F5 insert2, F6 clear2, Compare cmp = less{}){
	return matroid_intersection_weighted_spfa_base<T, F1, F2, F3, F4, F5, F6, Compare>(check1, insert1, clear1, check2, insert2, clear2, cmp);
}
