...
struct rigidity_matroid{
	...
	vector<int> was, prev;
	bool push(int u, int v, int t){
		if(!~pebble[u][t]) return true;
		++ it;
		prev[u] = -1;
		int id = pebble[u][t];
		was[u] = was[v] = was[u ^ edge[id][0] ^ edge[id][1]] = it;
		if((u ^ edge[id][0] ^ edge[id][1]) == v) return false;
		q[0] = {u, t};
	...
	auto make_matroid(){
		...
		auto check = [&](int i)->bool{
			...
			for(auto s = 0; s < 2; ++ s){
				int u = edge[i][s], v = edge[i][!s];
				for(auto t = 0; t < 2; ++ t) if(!push(u, v, t)) return false;
			}
			return true;
		};
		// O(n)
