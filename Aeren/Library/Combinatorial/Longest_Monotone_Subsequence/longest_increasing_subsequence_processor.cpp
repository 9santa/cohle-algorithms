...
struct longest_increasing_subsequence_processor{
	Compare cmp;
	...
	int n = 0;
	vector<T> sequence;
	vector<int> active; // position of the optimal last element among increasing sequences of length i+1
	vector<int> prev; // second-to-last element of the longest increasing sequence ending at i, -1 if non exists
	vector<int> length; // length of the longest increasing subsequence ending at i
	vector<U> count; // number of the longest increasing subsequences ending at i
	...
	longest_increasing_subsequence_processor(Compare cmp = less{}){ }
	longest_increasing_subsequence_processor(const vector<T> &a, Compare cmp = less{}){
		insert(a);
	}
	longest_increasing_subsequence_processor &operator=(const longest_increasing_subsequence_processor &lms){
		n = lms.n;
		sequence = lms.sequence;
		active = lms.active;
		prev = lms.prev;
		length = lms.length;
		count = lms.count;
