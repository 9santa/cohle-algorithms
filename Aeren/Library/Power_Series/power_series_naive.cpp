...
#define data (*this)
	...
	power_series_naive_base &inplace_antiderivative(){
		data.push_back(0);
		for(auto i = (int)data.size() - 1; i >= 1; -- i) data[i] = data[i - 1] * *T{i}.small_inverse();
		data[0] = 0;
		return *this;
	}
	// Integral(P)
	...
	power_series_naive_base &inplace_shifted_antiderivative(){
		...
		for(auto i = 1; i < data.size(); ++ i) data[i] *= *T{i}.small_inverse();
		return *this;
	}
	// Integral(P/x)
	...
	// O(degree() * n)
	template<class U, typename enable_if<IS_INTEGRAL(U)>::type* = nullptr>
	power_series_naive_base &inplace_power_with_differentiation(U e, int n){
		assert(n >= 0);
		if(n == 0){
			data.clear();
			return *this;
		...
		if(e < 0) return inplace_inverse(n).inplace_power(-e, n);
		assert(data[0] != T{0});
		power_series_naive_base res(n, T{0});
		res[0] = data[0].power(e);
		auto inv_ptr = data[0].inverse();
		assert(inv_ptr);
		auto inv = *inv_ptr;
		for(auto i = 1; i < n; ++ i){
			T x = e;
			for(auto j = 1; j < min((int)this->size(), i + 1); ++ j, x += e) res[i] += data[j] * res[i - j] * (x - (i - j));
			res[i] *= inv * *T{i}.small_inverse();
		}
		return *this = res;
	}
	// https://codeforces.com/blog/entry/73947?#comment-581173
	// O(degree() * n)
	template<class U, typename enable_if<IS_INTEGRAL(U)>::type* = nullptr>
	power_series_naive_base power_with_differentiation(U e, int n) const{
		return power_series_naive_base(*this).inplace_power_with_differentiation(e, n);
	}
	// O(n * log(n) * log(e))
	template<class U, typename enable_if<IS_INTEGRAL(U)>::type* = nullptr>
	...
	power_series_naive_base &inplace_set(int n){
		...
		for(auto i = 1; i < n; ++ i) (data[i] *= *T{i}.small_inverse()) *= (i & 1 ? 1 : -1);
	...
	power_series_naive_base &inplace_multiset(int n){
		assert(!data.empty() && data[0] == T{0});
		data.resize(n);
		for(auto i = n - 1; i >= 1; -- i) for(auto j = 2 * i; j < n; j += i) data[j] += data[i] * *T{j / i}.small_inverse();
		return inplace_exp(n);
	}
