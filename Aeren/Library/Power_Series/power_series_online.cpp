...
struct power_series_online_wrapper_base{
	...
	int make_const_series(const vector<T> &p, int u = -1, int v = -1){
		ASSERT(-1 <= u && u < (int)type.size());
		if(p.empty() || p[0] != T{0}){
			u = _new_series(u);
			...
			int zero_pref = 0;
			while(zero_pref < (int)p.size() && p[zero_pref] == T{0}) ++ zero_pref;
			u = make_const_series(vector<T>(p.begin() + zero_pref, p.end()), u);
			u = shift(u, zero_pref, v);
		}
		return u;
	}
	...
	int multiply(int v, int w, int u = -1){
		...
		ASSERT(-1 <= u && u < (int)type.size());
		ASSERT(0 <= min(v, w) && max(v, w) < (int)type.size() && type[v] != type_t::DEAD && type[w] != type_t::DEAD);
		if(type[v] == type_t::SHIFT) return shift(multiply(arg0[v], w), arg1[v], u);
		else if(type[w] == type_t::SHIFT) return shift(multiply(v, arg0[w]), arg1[w], u);
		else{
			u = _new_series(u);
