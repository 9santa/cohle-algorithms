...
#define self (*this)
	...
	power_series_base &inplace_antiderivative(){
		self.push_back(0);
		for(auto i = (int)self.size() - 1; i >= 1; -- i) self[i] = self[i - 1] * *T{i}.small_inverse();
		self[0] = 0;
		return *this;
	}
	// Integral(P)
	...
	power_series_base &inplace_shifted_antiderivative(){
		...
		for(auto i = 1; i < self.size(); ++ i) self[i] *= *T{i}.small_inverse();
		return *this;
	}
	// Integral(P/x)
	...
	// O(degree() * n)
	template<class U, typename enable_if<IS_INTEGRAL(U)>::type* = nullptr>
	power_series_base &inplace_power_with_differentiation(U e, int n){
		assert(n >= 0);
		if(n == 0){
			self.clear();
			return *this;
		...
		if(e < 0) return inplace_inverse(n).inplace_power(-e, n);
		assert(self[0] != T{0});
		power_series_base res(n, T{0});
		res[0] = self[0].power(e);
		auto inv_ptr = self[0].inverse();
		assert(inv_ptr);
		auto inv = *inv_ptr;
		for(auto i = 1; i < n; ++ i){
			T x = e;
			for(auto j = 1; j < min((int)this->size(), i + 1); ++ j, x += e) res[i] += self[j] * res[i - j] * (x - (i - j));
	...
	// O(degree() * n)
	template<class U, typename enable_if<IS_INTEGRAL(U)>::type* = nullptr>
	power_series_base power_with_differentiation(U e, int n) const{
		return power_series_base(*this).inplace_power_with_differentiation(e, n);
	}
	// O(n * log(n) * log(e))
	template<class U, typename enable_if<IS_INTEGRAL(U)>::type* = nullptr>
	...
	power_series_base &inplace_set(int n){
		...
		for(auto i = 1; i < n; ++ i) (self[i] *= *T{i}.small_inverse()) *= (i & 1 ? 1 : -1);
	...
	power_series_base &inplace_multiset(int n){
		assert(!self.empty() && self[0] == T{0});
		self.resize(n);
		for(auto i = n - 1; i >= 1; -- i) for(auto j = 2 * i; j < n; j += i) self[j] += self[i] * *T{j / i}.small_inverse();
		return inplace_exp(n);
	}
