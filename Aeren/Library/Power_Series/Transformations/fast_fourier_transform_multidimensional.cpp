	...
	static void transform(int n, T *p, bool invert = false){
		...
		if(!invert){
			...
			if constexpr(is_integral_v<T> || is_same_v<T, __int128_t> || is_same_v<T, __uint128_t>){
				T det = _transform_1D::determinant_squared(n);
				if(det != T{1}) for(auto i = 0; i < n; ++ i) p[i] /= det;
			}
			else{
				T inv = 1 / _transform_1D::determinant_squared(n);
...
struct _transform_1D_bitwise_xor{
	...
	static T determinant_squared(int n){
		return n;
	}
};
template<class T>
