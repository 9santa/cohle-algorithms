...
struct topological_sorter{
	...
	int attempt = -1;
	template<class T>
	optional<vector<int>> sort(const graph<T> &g, const vector<int> &subset){
		_init(g.n);
		order.clear();
		++ attempt;
		for(auto u: subset){
	...
	template<class T>
	optional<vector<int>> sort_all(const graph<T> &g){
		vector<int> subset(g.n);
		iota(subset.begin(), subset.end(), 0);
		return sort(g, subset);
	}
	template<class T, class Compare = less<>>
	optional<vector<int>> sort_lexicographically(const graph<T> &g, const vector<int> &subset, Compare cmp){
		_init(g.n);
		order.clear();
		++ attempt;
		for(auto u: subset){
	...
	template<class T, class Compare = less<>>
	optional<vector<int>> sort_all_lexicographically(const graph<T> &g, Compare cmp = less<>()){
		assert(g.n <= n);
		vector<int> subset(g.n);
		iota(subset.begin(), subset.end(), 0);
		return sort_lexicographically(g, subset, cmp);
	}
};
