...
struct wavelet_matrix_base{
	...
	// O(n * log(sigma)) time and O(n * log(sigma) / w) memory
	void build(int n, auto f_key, B sigma){
		static_assert(!HAS_QUERY);
		ASSERT(sigma > 0);
		vector<B> key(n);
		for(auto i = 0; i < n; ++ i) key[i] = f_key(i);
		build(key, sigma);
	}
	...
	// O(n * log(sigma)) time and O(n * log(sigma)) memory
	void build(int n, auto f_key, B sigma, auto f_value){
		static_assert(HAS_QUERY);
		ASSERT(sigma > 0);
		vector<B> key(n);
		vector<T> value(n);
		...
		return build(key, sigma, value);
	}
	// Returns the frequency of x in the interval [ql, qr)
	// O(log(sigma))
	int freq(int ql, int qr, B x) const{
	...
	// O(log(sigma))
	pair<int, T> freq_query(int ql, int qr, B x) const{
		static_assert(HAS_QUERY);
		ASSERT(0 <= ql && ql <= qr && qr <= n);
		ASSERT(0 <= x);
	...
	// pred(sum of values of items with prefix of key) is T, T, ..., T, F, F, ..., F
	// Returns {
	//  max x where summing every item with key < x gives T, or sigma if unbounded,
	//  how many more items with key x can it fit into sum while pred(sum) = T,
	//  sum of values, including extra items with key x
	// }
	// O(log(sigma))
	tuple<B, int, T> max_pref(int ql, int qr, auto pred) const{
		static_assert(HAS_QUERY);
		ASSERT(pred(T_id));
		B x = 0;
		T sum = T_id;
		for(auto h = lg; h --; ){
			auto lcnt = data[h].rank0(ql), rcnt = data[h].rank0(qr);
			T sum_next = TT(sum, TT(aggregate[h + 1][qr], Tinv(aggregate[h + 1][ql])));
			if(!pred(sum_next)) ql = lcnt, qr = rcnt;
			else{
				x |= (B)1 << h;
				sum = sum_next;
				ql += data[h].zeros - lcnt;
		...
		sum = TT(sum, Tinv(aggregate[0][ql]));
		int extra = *ranges::partition_point(views::iota(0, qr - ql + 1), [&](int extra){ return pred(TT(sum, aggregate[0][ql + extra])); }) - 1;
		ASSERT(extra >= 0);
		if(extra != qr - ql) return {x, extra, TT(sum, aggregate[0][ql + extra])};
		else return {sigma, 0, TT(sum, aggregate[0][qr])};
	}
...
auto make_wavelet_matrix(){
	...
	auto _Tinv = [&](){ assert(false); };
	return wavelet_matrix_base<false, B, T, decltype(_TT), decltype(_Tinv)>(_TT, T{}, _Tinv);
}
// Supports query
template<class B, class T = long long, class F = plus<>, class I = negate<>>
auto make_Q_wavelet_matrix(F TT = plus{}, T T_id = T{0}, I Tinv = negate{}){
