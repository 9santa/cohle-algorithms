...
#define IS_UNSIGNED(T) (is_unsigned_v<T> || is_same_v<T, __uint128_t>)
	...
	constexpr optional<montgomery_modular_fixed_base> small_inverse() const{
		...
		for(auto x = _inv.size(); x <= obj; ++ x){
			...
			if(_inv[rem]) _inv.push_back(_mod / x * -_inv[rem]);
				...
				auto inv_ptr = montgomery_modular_fixed_base{x}.inverse();
				_inv.push_back(inv_ptr ? *inv_ptr : montgomery_modular_fixed_base{0});
			}
		}
		return _inv[obj] ? _inv[obj] : optional<montgomery_modular_fixed_base>{};
	}
