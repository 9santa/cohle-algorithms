...
#define IS_UNSIGNED(T) (is_unsigned_v<T> || is_same_v<T, __uint128_t>)
	...
	static void setup(data_t mod = 0){
		...
		if(!mod) cin >> mod;
		_mod = mod;
		ASSERT(_mod % 2 == 1 && 1 <= _mod && _mod < data_t(1) << 8 * sizeof(data_t) - 1);
		_r = _mod;
	...
	optional<montgomery_modular_unfixed_base> small_inverse() const{
		if(_inv.empty()) _inv = {0, 1 % mod()};
		auto obj = data();
		for(auto x = _inv.size(); x <= obj; ++ x){
			auto rem = _mod % x;
			if(_inv[rem]) _inv.push_back(_mod / x * -_inv[rem]);
			else{
				auto inv_ptr = montgomery_modular_unfixed_base{x}.inverse();
				_inv.push_back(inv_ptr ? *inv_ptr : montgomery_modular_unfixed_base{0});
			}
		}
		return _inv[obj] ? _inv[obj] : optional<montgomery_modular_unfixed_base>{};
	}
...
using modular = montgomery_modular_unfixed_base<0, unsigned int, unsigned long long>;
// using modular = montgomery_modular_unfixed_base<0, unsigned long long, __uint128_t>;
