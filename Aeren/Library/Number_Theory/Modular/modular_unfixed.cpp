...
#define IS_UNSIGNED(T) (is_unsigned_v<T> || is_same_v<T, __uint128_t>)
	...
	optional<modular_unfixed_base> small_inverse() const{
		...
		for(auto x = _inv.size(); x <= obj; ++ x){
			...
			if(_inv[rem]) _inv.push_back(_mod / x * -_inv[rem]);
				...
				auto inv_ptr = modular_unfixed_base{x}.inverse();
				_inv.push_back(inv_ptr ? *inv_ptr : modular_unfixed_base{0});
			}
		}
		return _inv[obj] ? _inv[obj] : optional<modular_unfixed_base>{};
	}
