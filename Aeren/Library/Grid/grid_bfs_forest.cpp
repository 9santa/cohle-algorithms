...
struct grid_bfs_forest{
	...
	template<class Container, class F = equal_to<>>
	grid_bfs_forest(const vector<Container> &grid, const vector<array<int, 2>> &src, F propagate = equal_to<>()){ init((int)grid.size(), (int)grid[0].size()); build(grid, src, propagate); }
	...
	void _build(const vector<Container> &grid, const vector<int> &src, F propagate){
		q = src;
		for(auto u: src){
	...
	// O((# of cells reachable from src) * size(dr))
	template<class Container, class F = equal_to<>>
	void build(const vector<Container> &grid, const vector<array<int, 2>> &src, F propagate = equal_to<>()){
		...
		_build(grid, src_index, propagate);
	}
	// O(nr * nc * size(dr))
	template<class Container, class F = equal_to<>>
	void build_all(const vector<Container> &grid, F propagate = equal_to<>()){
		ASSERT((int)grid.size() <= nr);
		for(auto x = 0; x < nr; ++ x) ASSERT((int)grid[x].size() <= ncs[x]);
		root.clear(), order.clear();
		++ attempt;
		for(auto x = 0; x < (int)grid.size(); ++ x) for(auto y = 0; y < (int)grid[x].size(); ++ y) if(_was[index[x][y]] != attempt) _build(grid, {index[x][y]}, propagate);
	}
	// Check if (x, y) is visited during the last build-like call.
