...
struct LazyTreap {
    ...
    int min_left(const F check, int r) {
        assert(0 <= r && r <= size());
        assert(check(MX::id()));
        pair<Node*, Node*> ab = split(root, r);
        Node* a = ab.first;
        Node* b = ab.second;
...
private:
    ...
    pair<Node*, Node*> split(Node* t, int k) {
        ...
        if (size(t->l) >= k) {
            ...
            pair<Node*, Node*> ab = split(t->r, k - size(t->l) - 1);
            Node* a = ab.first;
            Node* b = ab.second;
            t->r = a;
            pull(t);
