...
struct Implicit_Treap_Beats {
    ...
    static pair<np, np> split(np t, int k) {
        ...
        if (k <= siz(t->l)) {
            ...
            pair<np, np> ab = split(t->r, k - siz(t->l) - 1);
            np a = ab.first;
            np b = ab.second;
            t->r = a;
            pull(t);
    ...
    static tuple<np, np, np> split3(np t, int l, int r) {
        pair<np, np> abc = split(t, r);     // ab: < r; c: >= r
        np ab = abc.first;
        np c = abc.second;
        pair<np, np> ab_split = split(ab, l);  // b: >= l, < r; a < l
        np a = ab_split.first;
        np b = ab_split.second;
        return {a, b, c};
    }
    ...
    void modifyi(int i, Op op) {
        assert(0 <= i && i < siz());
        pair<np, np> abc = split(root, i);
        np a = abc.first;
        np bc = abc.second;
        pair<np, np> bc_split = split(bc, 1);
        np b = bc_split.first;
        np c = bc_split.second;
    ...
    void reverse(int l, int r) {
        pair<np, np> abc = split(root, l);
        np a = abc.first;
        np bc = abc.second;
        pair<np, np> bc_split = split(bc, r - l);
        np b = bc_split.first;
        np c = bc_split.second;
        A tag = lid;
        tag.rev = true;
