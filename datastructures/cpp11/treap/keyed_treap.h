...
struct TreapKeyed {
    ...
    int max_right(int l, Pred pred) {
        ...
        pair<Node*, Node*> ab = split_by_order(root, l);
        Node* a = ab.first;
        Node* b = ab.second;
        X acc = Monoid::id();
    ...
    int min_left(int r, Pred pred) {
        assert(0 <= r && r <= size());
        assert(pred(Monoid::id()));
        pair<Node*, Node*> ab = split_by_order(root, r);
        Node* a = ab.first;
        Node* b = ab.second;
        X acc = Monoid::id();
...
private:
    ...
    pair<Node*, Node*> split_lt(Node* t, const Key& key) {
        ...
        if (comp(t->key, key)) { // node->key < key -> split the right subtree
            pair<Node*, Node*> ab = split_lt(t->r, key); // a: < key, b: >= key
            Node* a = ab.first;
            Node* b = ab.second;
            ...
            pair<Node*, Node*> ab = split_lt(t->l, key);
            Node* a = ab.first;
            Node* b = ab.second;
            t->l = b;
            pull(t);
    ...
    pair<Node*, Node*> split_leq(Node* t, const Key& key) {
        if (!t) return { nullptr, nullptr };
        if (leq(t->key, key)) {
            pair<Node*, Node*> ab = split_leq(t->r, key);
            Node* a = ab.first;
            Node* b = ab.second;
            t->r = a;
            pull(t);
            ...
            pair<Node*, Node*> ab = split_leq(t->l, key);
            Node* a = ab.first;
            Node* b = ab.second;
            t->l = b;
            pull(t);
    ...
    pair<Node*, Node*> split_by_order(Node* t, int k) {
        if (!t) return {nullptr, nullptr};
        if (size(t->l) >= k) {
            pair<Node*, Node*> ab = split_by_order(t->l, k);
            Node* a = ab.first;
            Node* b = ab.second;
            t->l = b;
            pull(t);
            ...
            pair<Node*, Node*> ab = split_by_order(t->r, k - size(t->l) - 1);
            Node* a = ab.first;
            Node* b = ab.second;
            t->r = a;
            pull(t);
    ...
    array<Node*, 3> split3_by_order(Node* t, int l, int r) {
        pair<Node*, Node*> abc = split_by_order(t, l);
        Node* a = abc.first;
        Node* bc = abc.second;
        pair<Node*, Node*> bc_split = split_by_order(bc, r-l);
        Node* b = bc_split.first;
        Node* c = bc_split.second;
        return {a, b, c};
    }
    ...
    vector<Node*> split_k_by_order(Node* t, const vector<int>& cuts) {
        ...
        int last = 0;
        for (int p : cuts) {
            pair<Node*, Node*> ab = split_by_order(cur, p - last);
            Node* a = ab.first;
            Node* b = ab.second;
            res.push_back(a);
            cur = b;
