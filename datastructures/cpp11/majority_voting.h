...
signed main(void) {
    ...
    // future updates
    // Replaced structured bindings with explicit array access for C++11.
    for (int i = 0; i < q; i++) {
        int t = queries[i][0], p = queries[i][1], x = queries[i][2];
    ...
    for (int qi = 0; qi < q; qi++) {
        int t = queries[qi][0], x = queries[qi][1], y = queries[qi][2];
        if (t == 0) { // update
            int old = a[x];
            if (old != y) {
