...
public:
    ...
    ModIntBase(T x_) : x(x_ % mod()) {} // reduce modulo P

    // handle negative numbers
    template<typename T, typename std::enable_if<std::is_signed<T>::value, int>::type = 0>
    ModIntBase(T x_) {
        // Replaced make_signed_t with typename std::make_signed<...>::type for C++11.
        typedef typename std::make_signed<U>::type S; // convert modulus to signed
    ...
    ModIntBase operator-() const {
        ModIntBase res;
        res.x = (x == 0 ? 0 : mod() - x);
        return res;
    }

    ModIntBase &operator*=(const ModIntBase &rhs) {
    ...
    friend ModIntBase operator+(ModIntBase lhs, const ModIntBase& rhs) {
        lhs += rhs;
        return lhs;
    }
    ...
    // Replaced C++20 spaceship comparison with ordinary comparisons for C++11.
