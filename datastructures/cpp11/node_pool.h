...
struct NodePool {
    ...
    struct Slot {
        union { // union -> only one member "active" at a time
            ...
            // Replaced std::byte + aligned new with C++11 aligned_storage.
            typename std::aligned_storage<sizeof(Node), alignof(Node)>::type storage; // when occupied, raw bytes to host a Node
        };
    };
...
private:
    void add_chunk() {
        // Replaced aligned operator new with plain operator new on aligned_storage slots for C++11.
