...
struct Monoid_Sum {
    ...
    // Replaced if constexpr + _v type trait aliases with C++11 dispatch.
    static X op(const X& x, const X& y) {
        return op_impl(x, y, typename std::is_arithmetic<E>::type());
    }

    static X id() {
        return id_impl(typename std::is_arithmetic<E>::type());
    }

private:
    static X op_impl(const X& x, const X& y, std::true_type) {
        return x + y;
    }

    static X op_impl(const X& x, const X& y, std::false_type) {
        X res = x;
    ...
    static X id_impl(std::true_type) {
        return E(0);
    }

    static X id_impl(std::false_type) {
        X res{};
