...
// Range updates via act(), range query via opX()
// Monoid X: How do my data nodes combine? (The "Result")
// Monoid A: How do my updates combine? (The "Lazy Tag")
// The action act(): How does an update transform a data node?
type LazySegtree[X any, A any] struct {
	n    int
	log  int
...
func NewLazySegtree[X any, A any](
	n int,
	opX func(X, X) X,
	idX func() X,
	opA func(A, A) A,
...
func NewLazySegtreeFromSlice[X any, A any](
	v []X,
	opX func(X, X) X,
	idX func() X,
	opA func(A, A) A,
...
func (st *LazySegtree[X, A]) Build(m int, f func(int) X) {
	st.n = m
	st.log = 1
	for (1 << st.log) < st.n {
		st.log++
...
func (st *LazySegtree[X, A]) update(p int) {
...
func (st *LazySegtree[X, A]) Set(p int, x X) {
	if !(0 <= p && p < st.n) {
...
func (st *LazySegtree[X, A]) Multiply(p int, x X) {
	if !(0 <= p && p < st.n) {
...
func (st *LazySegtree[X, A]) Get(p int) X {
	if !(0 <= p && p < st.n) {
...
func (st *LazySegtree[X, A]) GetAll() []X {
	for i := 1; i < st.size; i++ {
		st.push(i)
	}
	res := make([]X, st.n)
...
func (st *LazySegtree[X, A]) Prod(l, r int) X {
	if !(0 <= l && l <= r && r <= st.n) {
...
// whole tree
func (st *LazySegtree[X, A]) ProdAll() X {
	return st.data[1]
}

func (st *LazySegtree[X, A]) Apply(l, r int, a A) {
	if !(0 <= l && l <= r && r <= st.n) {
...
func (st *LazySegtree[X, A]) MaxRight(check func(X) bool, l int) int {
	if !(0 <= l && l <= st.n) {
...
func (st *LazySegtree[X, A]) MinLeft(check func(X) bool, r int) int {
	if !(0 <= r && r <= st.n) {
...
func (st *LazySegtree[X, A]) applyAt(p int, a A) {
...
func (st *LazySegtree[X, A]) push(p int) {
